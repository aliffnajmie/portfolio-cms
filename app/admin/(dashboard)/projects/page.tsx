...
export default async function AdminProjectsPage() {
  ...
  return <main className="p-6 sm:p-8 lg:p-10"><div className="mx-auto max-w-6xl"><div className="flex flex-col gap-5 border-b border-white/10 pb-8 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">Content</p><h1 className="text-3xl font-semibold tracking-tight text-white sm:text-4xl">Projects</h1><p className="mt-3 text-slate-400">Manage the work displayed across your portfolio.</p></div><Link href="/admin/projects/new" className="button-primary justify-center"><Plus size={17}/>New project</Link></div>
    ...
    : <><div className="mt-8 hidden overflow-hidden border border-white/10 md:block"><table className="w-full text-left"><thead className="border-b border-white/10 bg-white/[0.025] font-mono text-[10px] uppercase tracking-[.14em] text-slate-500"><tr><th className="px-5 py-4 font-medium">Project</th><th className="px-5 py-4 font-medium">Status</th><th className="px-5 py-4 font-medium">Technologies</th><th className="px-5 py-4 text-center font-medium">Order</th><th className="px-5 py-4 text-right font-medium">Updated</th><th className="px-5 py-4 text-right font-medium">Action</th></tr></thead><tbody className="divide-y divide-white/10">{projects.map((project) => <tr key={project.id} className="bg-slate-950/20"><td className="px-5 py-5"><p className="font-medium text-white">{project.title}</p>{project.featured && <span className="mt-2 inline-block border border-cyan-300/20 px-2 py-0.5 text-[10px] uppercase tracking-wider text-cyan-300">Featured</span>}</td><td className="px-5 py-5"><StatusBadge status={project.status}/></td><td className="max-w-xs px-5 py-5"><Technologies items={project.technologies}/></td><td className="px-5 py-5 text-center font-mono text-sm text-slate-400">{project.display_order}</td><td className="whitespace-nowrap px-5 py-5 text-right text-sm text-slate-500">{dateFormatter.format(new Date(project.updated_at))}</td><td className="px-5 py-5 text-right"><Link href={`/admin/projects/${project.id}/edit`} aria-label={`Edit ${project.title}`} className="inline-flex items-center gap-2 border border-white/10 px-3 py-2 text-xs font-medium text-slate-300 transition hover:border-cyan-300/30 hover:text-cyan-300"><Pencil size={14}/>Edit</Link></td></tr>)}</tbody></table></div>
    <div className="mt-8 space-y-4 md:hidden">{projects.map((project) => <article key={project.id} className="border border-white/10 bg-slate-950/25 p-5"><div className="flex items-start justify-between gap-4"><div><h2 className="font-medium text-white">{project.title}</h2><div className="mt-3 flex flex-wrap items-center gap-2"><StatusBadge status={project.status}/>{project.featured && <span className="border border-cyan-300/20 px-2 py-1 text-[10px] uppercase tracking-wider text-cyan-300">Featured</span>}</div></div><span className="font-mono text-xs text-slate-500">#{project.display_order}</span></div><div className="mt-5"><Technologies items={project.technologies}/></div><div className="mt-5 flex items-center justify-between border-t border-white/10 pt-4"><p className="text-xs text-slate-500">Updated {dateFormatter.format(new Date(project.updated_at))}</p><Link href={`/admin/projects/${project.id}/edit`} aria-label={`Edit ${project.title}`} className="inline-flex items-center gap-2 px-2 py-1 text-xs font-medium text-cyan-300"><Pencil size={14}/>Edit</Link></div></article>)}</div></>}
  </div></main>;
}
