...
export function ProjectForm({ action, initialValues = emptyProjectFormValues, mode = "create" }: { action: ProjectFormAction; initialValues?: ProjectFormValues; mode?: "create" | "edit" }) {
  ...
  return <form action={formAction} className="space-y-8" noValidate>
    ...
    <div className="flex flex-col-reverse gap-3 border-t border-white/10 pt-6 sm:flex-row sm:justify-end"><Link href="/admin/projects" className="button-secondary justify-center">Cancel</Link><button type="submit" disabled={pending} className="button-primary min-w-36 justify-center disabled:cursor-not-allowed disabled:opacity-60">{pending ? <><LoaderCircle className="animate-spin" size={17}/>{mode === "edit" ? "Saving..." : "Creating..."}</> : mode === "edit" ? "Save changes" : "Create project"}</button></div>
  </form>;
}
