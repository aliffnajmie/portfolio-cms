...
function Field({ name, label, error, required, hint, children }: FieldProps) {
  ...
  return <div><label htmlFor={name} className="mb-2 block text-sm font-medium text-foreground">{label}{required && <span className="ml-1 text-primary" aria-hidden="true">*</span>}</label>{hint && <p id={`${name}-hint`} className="mb-2 text-xs text-muted-foreground">{hint}</p>}<div data-describedby={describedBy}>{children}</div>{error?.[0] && <p id={`${name}-error`} role="alert" className="mt-2 text-sm text-red-300">{error[0]}</p>}</div>;
}

const inputClass = "h-12 w-full rounded-md border border-input bg-background/45 px-4 text-sm text-foreground shadow-sm outline-none transition placeholder:text-muted-foreground focus:border-primary/60 focus:ring-2 focus:ring-primary/20 disabled:opacity-60";
...
export function ProjectForm({ action, initialValues = emptyProjectFormValues, mode = "create" }: { action: ProjectFormAction; initialValues?: ProjectFormValues; mode?: "create" | "edit" }) {
  ...
  return <form action={formAction} className="space-y-8" noValidate onSubmit={(event) => { if (thumbnailClientError) event.preventDefault(); }}>
    ...
    <section className="surface rounded-lg p-5 sm:p-7"><div className="mb-7"><h2 className="text-lg font-semibold">Project details</h2><p className="mt-1 text-sm text-muted-foreground">The core information used to identify and describe this project.</p></div><div className="grid gap-6 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2"><Field name="summary" label="Summary" required error={errors.summary}><Textarea id="summary" name="summary" rows={3} defaultValue={initialValues.summary} required maxLength={500} disabled={pending} aria-invalid={Boolean(errors.summary)} aria-describedby={errors.summary ? "summary-error" : undefined}/></Field></div>
      <div className="sm:col-span-2"><Field name="content" label="Content" hint="Optional longer project description." error={errors.content}><Textarea id="content" name="content" rows={8} defaultValue={initialValues.content} disabled={pending}/></Field></div>
    ...
    <section className="surface rounded-lg p-5 sm:p-7"><div className="mb-7"><h2 className="text-lg font-semibold">Links and media</h2><p className="mt-1 text-sm text-muted-foreground">Optional destinations and a project thumbnail.</p></div><div className="grid gap-6 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2">
        <Field name="thumbnail_image" label="Project thumbnail" hint="JPEG, PNG or WebP. Maximum file size: 5 MB." error={thumbnailErrors}>
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
            <div>
              <input ref={thumbnailInput} id="thumbnail_image" name="thumbnail_image" type="file" accept="image/jpeg,image/png,image/webp" className={`${inputClass} h-auto cursor-pointer py-3 file:mr-4 file:rounded file:border-0 file:bg-primary file:px-3 file:py-2 file:text-xs file:font-semibold file:text-primary-foreground`} disabled={pending} aria-invalid={Boolean(thumbnailErrors)} aria-describedby={thumbnailErrors ? "thumbnail_image-error thumbnail_image-hint" : "thumbnail_image-hint"} onChange={handleThumbnailChange}/>
              {initialValues.thumbnail_url && <label className="mt-3 flex cursor-pointer items-center gap-3 text-sm text-foreground"><input name="remove_thumbnail" type="checkbox" checked={removeThumbnail} onChange={(event) => handleRemoveThumbnail(event.target.checked)} disabled={pending} className="size-4 accent-[hsl(var(--primary))]"/>Remove existing image</label>}
            </div>
            <div className="relative aspect-video overflow-hidden rounded-md border border-border bg-muted/40">
              {thumbnailPreview ? <Image src={thumbnailPreview} alt={`${initialValues.title || "Project"} thumbnail preview`} fill sizes="(min-width: 1024px) 18rem, 100vw" className="object-cover" unoptimized/> : <div className="flex h-full items-center justify-center px-4 text-center text-xs text-muted-foreground">No thumbnail selected</div>}
            </div>
          </div>
        </Field>
      </div>
    </div></section>
    <section className="surface rounded-lg p-5 sm:p-7"><div className="mb-7"><h2 className="text-lg font-semibold">Publishing</h2><p className="mt-1 text-sm text-muted-foreground">Control visibility and placement in the portfolio.</p></div><div className="grid gap-6 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2"><label className="flex cursor-pointer items-start gap-3 rounded-md border border-border bg-muted/25 p-4"><input name="featured" type="checkbox" defaultChecked={initialValues.featured} className="mt-0.5 size-4 accent-[hsl(var(--primary))]" disabled={pending}/><span><span className="block text-sm font-medium">Featured project</span><span className="mt-1 block text-xs text-muted-foreground">Highlight this project in featured areas of the portfolio.</span></span></label></div>
    </div></section>
